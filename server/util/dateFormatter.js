...
export function formattedSameDate(startData, endDate) {
  const dateTime1 = transDateType(
    startData?.replace('년', '-')?.replace('월', '-')?.replace('일', '')?.replace(/\s/gi, '')
  );
...
export const timePassIndicator = (criterionDate, compareDate) => {
  ...
  if (subtraction <= 1000 * 60 * 60 * 24) {
    return '오늘';
  } else if (subtraction >= 1000 * 60 * 60 * 24 && subtraction < 1000 * 60 * 60 * 24 * 2) {
    return '어제';
  } else if (subtraction >= 1000 * 60 * 60 * 24 * 2 && subtraction < 1000 * 60 * 60 * 24 * 3) {
    return '그저께';
  } else {
...
export const timeLeftIndicator = (criterionDayLength, compareDate) => {
  const subtraction = compareDate.getTime() + criterionDayLength * 1000 * 60 * 60 * 24;
